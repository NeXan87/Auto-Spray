...
#define PIN_BATTERY_POINT A1  // Вход делителя
#define PIN_SW_GLOBAL_EN A2   // Цифровой выключатель работы
#define PIN_KEY_BAT_EN A3     // Включение делителя напряжения
...
#define PIN_SW_SPRAY_1 7      // переключатель пшиков: 1 пшик
#define PIN_SW_SPRAY_2 8      // переключатель пшиков: 2 пшика (3 пшика — когда ЗАМКНУТЫ ОБА: D7 И D8)
#define PIN_SW_MODE 9         // Переключатель режима (LOW = пшик после выключения света)
...
// -----------------------------------------------------------
// КОНТРОЛЬ НАПРЯЖЕНИЯ БАТАРЕИ (Li-ion, 3.0–4.2 В)
// -----------------------------------------------------------
#define VCC_ARDUINO 3000.0f                // напряжение питания Ардуино (мВ)
#define BATTERY_DIVIDER_RATIO 1.43f        // коэффициент делителя
#define BATTERY_LOW_MV 3500UL              // напряжение разряженной батареи
#define BATTERY_BLOCKED_MV 3200UL          // напряжение при котором блокируется работа устройства
#define BATTERY_CHECK_INTERVAL_MS 10000UL  // период обновления заряда батареи
#define BATTERY_LED_ON_MS 100UL            // длительность вспышки
#define BATTERY_LED_OFF_MS 500UL           // период выключенного светодиода

// -----------------------------------------------------------
// ТАЙМИНГИ
// -----------------------------------------------------------
#define LIGHT_READY_MINUTES 2          // Время ожидания света до готовности
#define BLOCK_MINUTES 15               // Время блокировки после пшика
#define LIGHT_READY_MS 2000UL          // (LIGHT_READY_MINUTES * 60UL * 1000UL)        // Время, которое свет должен быть включён, прежде чем устройство перейдёт в режим READY
#define BLOCK_MS 5000UL                // (BLOCK_MINUTES * 60UL * 1000UL)               // Время блокировки после распыления
#define LIGHT_READ_INTERVAL_MS 1000UL  // Опрос датчика света

// -----------------------------------------------------------
// ПАРАМЕТРЫ РАСПЫЛЕНИЯ
// -----------------------------------------------------------
#define SPRAY_REVERSE_EN false     // true = включить реверс, false = отключить
#define SPRAY_ON_MS 500UL       // длительность работы двигателя для пшика (мс)
#define SPRAY_PAUSE1_MS 300UL   // пауза между реверсом двигателя (мс)
#define SPRAY_REVERSE_MS 200UL  // длительность реверса двигателя (мс)
#define SPRAY_PAUSE2_MS 1000UL   // пауза между пшиками (мс)
...
// -----------------------------------------------------------
#define ACTIVITY_LED_ENABLED false  // включить/выключить светодиод (для дебага)
...
// РЕЖИМ СНА
// -----------------------------------------------------------
#define ENABLE_SLEEP_MODE true  // true = включить сон, false = отключить
